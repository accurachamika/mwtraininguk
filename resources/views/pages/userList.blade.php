        ...
        <div class="row g-4">
            ...
            <div class="card-content text-end">
                ...
                <div class=" mt-2 text-end">
                    ...
                    <a href="{{ route('truncateUsers') }}" class="btn btn-outline-danger"
                        onclick="return confirm('Are you sure you want to delete all users? This action cannot be undone.');">Delete
                        All Users</a>
                        ...
                        <tbody>
                            @foreach ($users as $user)
                                <tr>
                                    ...
                                    <td>
                                        @if ($user->active !== 0)
                                            <a href="{{ route('acc_activate', ['id' => $user->user_id]) }}"
                                                class="btn btn-danger px-2 py-1 badge {{ $user->user_type === 'admin' ? 'disabled' : '' }}">Deactivate</a>
                                        @else
                                            <a href="{{ route('acc_activate', ['id' => $user->user_id]) }}"
                                                class="btn btn-success px-2 py-1 badge">Activate</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
        ...
        <script>
            $(document).ready(function() {
                $('#data-table').DataTable({
                    ...
                    "lengthChange": false // Enable the ability to change page length
                });
            });
        </script>
    @endif
